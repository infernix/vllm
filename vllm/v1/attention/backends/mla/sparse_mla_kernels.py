...
@triton.jit
def _accumulate_fp8ds_local_slots_attention_chunk_multihead_kernel(
    q_ptr,
    k_cache_ptr,
    local_indices_ptr,
...
def accumulate_fp8ds_local_slots_sparse_mla_attention_chunk_multihead(
    q: torch.Tensor,
    k_cache: torch.Tensor,
    ...
    assert q.dim() == 3, f"Expected q shape [T, H, D], got {q.shape}"
    assert q.shape[-1] == 512
    assert local_indices.dim() == 2
    assert local_indices.shape[0] == q.shape[0]
    assert token_to_req_indices.shape[0] == q.shape[0]
    ...
    block_d = min(1024, triton.next_power_of_2(head_dim))
    grid = (num_tokens, triton.cdiv(num_heads, head_block_size))
    _accumulate_fp8ds_local_slots_attention_chunk_multihead_kernel[grid](
        q,
        k_cache,
        local_indices,
...
@triton.jit
def _accumulate_fp8ds_swa_slots_attention_chunk_multihead_kernel(
    q_ptr,
    k_cache_ptr,
    token_to_req_indices_ptr,
    query_start_loc_ptr,
...
def accumulate_fp8ds_swa_slots_sparse_mla_attention_chunk_multihead(
    q: torch.Tensor,
    k_cache: torch.Tensor,
    token_to_req_indices: torch.Tensor,
    ...
    block_d = min(1024, triton.next_power_of_2(head_dim))
    grid = (num_tokens, triton.cdiv(num_heads, head_block_size))
    _accumulate_fp8ds_swa_slots_attention_chunk_multihead_kernel[grid](
        q,
        k_cache,
        token_to_req_indices,
        query_start_loc,
